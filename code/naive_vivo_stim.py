...
for slice_seed in slice_seeds:
    ...
    for index, row in sorted_results.iterrows():
        ...
        for stim_seeds in np.arange(1, N_stim_seeds + 1):
            ...
            np.random.seed(int(slice_seed * row["Sim"] + stim_seeds))
            xs, ys, lbound, ubound, zs = expan.ephys_sample(
                xd1["Conductance"], xd1["Time constant"]
            )
            ...
            _, connections, total_weight = pmr.find_connections(size, sim_to_pull_from)
    ...
    for index, row in sorted_results.iterrows():
        ...
        for stim_seeds in np.arange(1, N_stim_seeds + 1):
            ...
            np.random.seed(int(slice_seed * row["Sim"] + stim_seeds + 3))

            wstrs = np.zeros(size)
            tausyn_dend_dist = np.zeros(size)

            xs, ys, lbound, ubound, zs = expan.ephys_sample(
                xgpe["Conductance"], xgpe["Time constant"]
            )
